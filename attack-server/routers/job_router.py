...
async def start_singleattack_job(body: SingleAttackProps = Body(...)) -> Response:
    ...
    try:
        ...
        model = [m["name"] for m in model_response if m["name"] == id_model]
        model_type = [m["type"] for m in model_response if m["name"] == id_model]

        if len(model) > 1 or len(model_type) > 1:
            logging.error(f"The provided model string {id_model} has had more than one match. Check model repository")
            return Response(
                status_code=409,
                content=Error(code=409,
                              message=f"The provided model string {id_model} has had more than one match. Check model repository").model_dump_json())

        if len(model) == 0 or len(model_type) == 0:
            logging.error(f"No model found: {id_model}")
            return Response(
                status_code=404,
                content=Error(code=404,
                              message=f"No model found: {id_model}").model_dump_json())

        file_path = os.path.join(os.environ.get('INTERNAL_MODEL_STORAGE'), f"{id_model}.json")
        ...
        if model_type[0] == "saved_model":
            if os.path.exists(file_path) and os.path.isfile(file_path):
                ...
                logging.error(f"Can't find model metadata in the repository for model:{id_model}")
                return Response(
                    status_code=404,
                    content=Error(code=404,
                                  message=f"Can't find model metadata in the repository for model:{id_model}").model_dump_json())
        elif model_type[0] == "timm":
            model_metadata = None
            for m in model_response:
                if m["name"] == id_model:
        ...
        if model_type[0] == "saved_model":
            config_models["weights_path"] = os.path.join(os.environ.get('INTERNAL_MODEL_STORAGE'), f"{id_model}.pth")
