...
class AdversarialReportGenerator:
    ...
    def __init__(self, logo_path=None, generated_by_entity=None):
        """
        ...
        self.generated_by_entity = generated_by_entity
    ...
    def _build_title_page(self, data):
        ...
        # Report metadata
        generator_entity_string = f"\n<b>Generated By:</b> {self.generated_by_entity} <br/>\n" if self.generated_by_entity else ""
        metadata_text = f"""
        <font size=10>
        <b>Report Date:</b> {datetime.now().strftime('%B %d, %Y')}<br/>{generator_entity_string}
        </font>
        """
...
def create_benchmark_report(benchmark_run_dir: str | pathlib.Path,
    ...
    Parameters
    ----------
        benchmark_run_dir : str | pathlib.Path
            Root directory containing benchmark results for multiple datasets and models.
        model_name : str
            Name of the model for which the report is generated.
        dataset_name : str
            Name of the dataset associated with the benchmark run.
        filename : str, optional
            path where the final report files will be written. Defaults to
            ...
            if stat_file.exists() and stat_file.is_file():
                try:
                    with open(stat_file, "r", encoding="utf-8") as sf:
                        ...
                        sf_data["power"] = 0.5
                        statistics[attack_dir.name] = sf_data
                except Exception as e:
                    logging.warning(f"Could not load statistics.json for attack '{attack_dir.name}'({stat_file}). Raised Exception: {e}")
